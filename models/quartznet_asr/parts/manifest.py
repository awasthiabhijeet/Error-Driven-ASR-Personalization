        ...
        for manifest_path in manifest_paths:
            with open(manifest_path, "r", encoding="utf-8") as fh:
                ...
                for data in a:
                    ...
                    data['audio_filepath'] = audio_paths
                    data.pop('files')
...
class NeMoManifest(object):
    def __init__(self, data_dir, manifest_paths, labels, blank_index, max_duration=None, pad_to_max=False,
                 min_duration=None, sort_by_duration=False, max_utts=0,
                 normalize=True, speed_perturbation=False, filter_speed=1.0, 
                 read_offsets=False, read_pairs=False):
        self.labels_map = dict([(labels[i], i) for i in range(len(labels))])
        self.blank_index = blank_index
        self.max_duration= max_duration
        ids = []
            ...
            with open(manifest_path, "r") as fh:
                for line in fh:
                    ...
                    if 'original_duration' in data:
                        instance_duration = data['original_duration']
                    else:
                        instance_duration = data['duration']

                    assert not isinstance(instance_duration, list)
                    if min_duration is not None and instance_duration < min_duration:
                        print('filtering due to < min_duration: ',line)
                        filtered_duration += instance_duration
                        continue
                    if max_duration is not None and instance_duration > max_duration:
                        print('filtering due to > max_duration: ',line)
                        filtered_duration += instance_duration
                        continue
                    ...
                    data.pop('original_duration')
                    '''
                    if not isinstance(data['audio_filepath'],list):
                        data['audio_filepath'] = os.path.join(data_dir,data['audio_filepath'])
                        data['audio_filepath'] = [data['audio_filepath']]
                    if not isinstance(data['duration'],list):
                        data['audio_duration'] = [data['duration']]
                    else:
                        data['audio_duration']=data['duration']

                    if read_offsets:
                        assert 'target_offsets' in data
        ...
        if read_pairs:
            #print(len(ids))
            assert len(ids)%2 == 0
            ids = [ids[i:i+2] for i in range(0,len(ids),2)]
